...
ZIPNAME="Onclite-$(date '+%Y%m%d-%H%M').zip"
TC_DIR="$HOME/proton-clang-build/install"
DEFCONFIG="onlite-perf_defconfig"
...
make -j$(nproc --all) O=out ARCH=arm64 CC="ccache clang" AR=llvm-ar NM=llvm-nm OBJCOPY=llvm-objcopy OBJDUMP=llvm-objdump STRIP=llvm-strip CROSS_COMPILE=aarch64-linux-gnu- CROSS_COMPILE_ARM32=arm-linux-gnueabi- Image.gz-dtb dtbo.img
fi
